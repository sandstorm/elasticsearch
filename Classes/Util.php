...
class Tx_Elasticsearch_Util {
	...
	public static function getSiteHash($pageId = 0) {
		$site = Tx_Elasticsearch_Site::getSiteByPageId($pageId);
	...
	public static function getSolrConfiguration() {
		...
		return $GLOBALS['TSFE']->tmpl->setup['plugin.']['tx_elasticsearch.'];
	}
	...
	public static function getSolrConfigurationFromPageId($pageId, $initializeTsfe = FALSE, $language = 0) {
		...
		if ($initializeTsfe) {
			...
			$configurationCache[$cacheId] = $GLOBALS['TSFE']->tmpl->setup['plugin.']['tx_elasticsearch.'];
		} else {
			if (!isset($configurationCache[$cacheId])) {
				...
				$solrConfiguration = $tmpl->ext_getSetup($tmpl->setup, 'plugin.tx_elasticsearch');
