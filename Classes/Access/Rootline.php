 ...
 * @subpackage	solr
 */
class Tx_Elasticsearch_Access_Rootline {
	 ...
	 *
	 * @param	Tx_Elasticsearch_Access_RootlineElement	$rootlineElement Element to add.
	 */
	public function push(Tx_Elasticsearch_Access_RootlineElement $rootlineElement) {
		$lastElementIndex = max(0, (count($this->rootlineElements) - 1));

		if (!empty($this->rootlineElements[$lastElementIndex])) {
			if ($this->rootlineElements[$lastElementIndex]->getType() == Tx_Elasticsearch_Access_RootlineElement::ELEMENT_TYPE_CONTENT) {
				throw new Tx_Elasticsearch_Access_RootlineElementFormatException(
					'Can not add an element to an Access Rootline whose\' last element is a content type element.',
					1294422132
				);
			}

			if ($this->rootlineElements[$lastElementIndex]->getType() == Tx_Elasticsearch_Access_RootlineElement::ELEMENT_TYPE_RECORD) {
				throw new Tx_Elasticsearch_Access_RootlineElementFormatException(
	...
	public static function getAccessRootlineByPageId($pageId) {
		$accessRootline = t3lib_div::makeInstance('Tx_Elasticsearch_Access_Rootline');
		...
		foreach ($rootline as $pageRecord) {
			if ($pageRecord['fe_group']
				...
				$accessRootline->push(t3lib_div::makeInstance(
					'Tx_Elasticsearch_Access_RootlineElement',
					$pageRecord['uid'] . Tx_Elasticsearch_Access_RootlineElement::PAGE_ID_GROUP_DELIMITER . $pageRecord['fe_group']
				));
			}
		}
		...
		if ($currentPageRecord['fe_group']) {
			$accessRootline->push(t3lib_div::makeInstance(
				'Tx_Elasticsearch_Access_RootlineElement',
				$currentPageRecord['uid'] . Tx_Elasticsearch_Access_RootlineElement::PAGE_ID_GROUP_DELIMITER . $currentPageRecord['fe_group']
			));
		}
